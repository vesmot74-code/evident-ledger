...
use crate::sac::{SacDocument, SacTarget, SacTsaStatus, SacVerificationStatus};
use printpdf::*;
use std::io::Cursor;
...
fn write_exclusions_section(ctx: &mut PdfCtx) {
    ...
    ctx.gap();
    ctx.line(
        "[PASS] Existence (or absence) of a registered ledger state for the",
        10.0,
    );
    ctx.line("       Chain ID above", 10.0);
    ctx.line(
        "[PASS] Merkle Root, if present, matches the record held by the",
        10.0,
    );
    ctx.line("       Evident Ledger at issuance time", 10.0);
    ctx.line(
        "[PASS] Accompanying signature, if present, is valid for the stated",
        10.0,
    );
    ctx.line("       public key", 10.0);
    ctx.line(
        "[PASS] Presence or absence of an external RFC3161 timestamp",
        10.0,
    );
    ctx.gap();
    ...
    ctx.line(
        "[N/A]  Authorship of the underlying documents or events",
        10.0,
    );
    ctx.line("[N/A]  Legal interpretation of the recorded data", 10.0);
    ctx.line(
        "[N/A]  Guarantee that the ledger state above will remain",
        10.0,
    );
...
fn write_footer_section(ctx: &mut PdfCtx, chain_id: &str) {
    ...
    ctx.gap();
    ctx.line(
        "Verification performed by: Evident Ledger Verification Service",
        9.0,
    );
    ctx.line(
        &format!(
            "Verify independently at:   /verify/{}/attestation",
            chain_id
        ),
        9.0,
    );
    ...
    ctx.gap();
    ctx.line(
        "This certificate corresponds to the Merkle Root cited in the",
        9.0,
    );
    ctx.line(
        "Evident Report for this Chain ID, if one has been issued.",
        9.0,
    );
...
pub fn render_sac_pdf(doc: &SacDocument) -> Vec<u8> {
    ...
    ctx.gap();
    ctx.line(
        &format!("SAC Certificate Format v{}", SAC_PDF_VERSION),
        10.0,
    );
    ctx.gap();
    ...
    if matches!(doc.verification.status, SacVerificationStatus::NotFound) {
        ...
        ctx.gap();
        ctx.line(
            "This certificate attests that no ledger record exists for the",
            10.0,
        );
        ...
        ctx.gap();
        ctx.line(
            "No further sections apply. See \"Verification Scope\" below.",
            10.0,
        );
    ...
    match doc.verification.status {
        SacVerificationStatus::Verified => {
            ...
            ctx.gap();
            ctx.line(
                "Integrity and authenticity of the recorded evidence confirmed.",
                9.5,
            );
        }
    ...
    match &doc.tsa {
        Some(tsa) if matches!(tsa.status, SacTsaStatus::Present) => {
            ...
            ctx.line(
                &format!(
                    "Timestamp:  {}",
                    tsa.timestamp.map(|t| t.to_string()).unwrap_or_default()
                ),
                10.0,
            );
        ...
        _ => {
            ...
            ctx.gap();
            ctx.line(
                "No external RFC3161 timestamp is attached to this record.",
                10.0,
            );
            ctx.line(
                "This does not affect the validity of the ledger signature above.",
                10.0,
            );
        }
    }
