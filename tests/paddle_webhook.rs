...
fn payment_failed_payload(event_id: &str, customer_id: &str, subscription_id: &str) -> String {
    json!({
        "event_id": event_id,
...
#[tokio::test]
async fn subscription_payment_failed_sets_past_due() {
    let pool = test_pool().await;
    let customer_id = format!("ctm_{}", Uuid::new_v4());
    let subscription_id = format!("sub_{}", Uuid::new_v4());
    ...
    let body = payment_failed_payload(
        &format!("evt_{}", Uuid::new_v4()),
        &customer_id,
        &subscription_id,
    );
...
#[tokio::test]
async fn subscription_payment_succeeded_sets_active() {
    let pool = test_pool().await;
    let customer_id = format!("ctm_{}", Uuid::new_v4());
    let subscription_id = format!("sub_{}", Uuid::new_v4());
    ...
    let body = json!({
        ...
        "data": {
            "id": subscription_id,
            "customer_id": customer_id,
            "current_billing_period": { "ends_at": "2026-09-18T10:00:00Z" }
        }
    })
    .to_string();
